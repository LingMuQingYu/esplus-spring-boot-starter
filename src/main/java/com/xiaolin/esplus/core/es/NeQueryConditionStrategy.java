...
public class NeQueryConditionStrategy implements EsQueryCondition {
    private static volatile EsQueryCondition singleton;

    private NeQueryConditionStrategy(){}

    public static EsQueryCondition getInstance() {
        if (singleton == null) {
            synchronized (NeQueryConditionStrategy.class) {
                if (singleton == null) {
                    singleton = new NeQueryConditionStrategy();
                }
            }
        }
        return singleton;
