...
class CriteriaFilterProcessor {
	...
	public static Optional<Query> createQuery(Criteria criteria) {
		...
		for (Criteria chainedCriteria : criteria.getCriteriaChain()) {
			...
			if (chainedCriteria.isOr()) {
				...
				if (chainedCriteria.getField() != null && !chainedCriteria.getFilterCriteriaEntries().isEmpty()) {
					Collection<? extends Query> negatingFilters = buildNegatingFilter(chainedCriteria.getField().getName(),
							chainedCriteria.getFilterCriteriaEntries());
	...
	private static Collection<? extends Query> buildNegatingFilter(String fieldName,
		...
		filterCriteriaEntries.forEach(criteriaEntry -> {
			...
			if (query.isPresent()) {
				BoolQuery negatingFilter = co.elastic.clients.elasticsearch._types.query_dsl.QueryBuilders.bool().mustNot(query.get()).build();
	...
	private static ObjectBuilder<GeoDistanceQuery> withinQuery(String fieldName, Object... values) {
		...
		return co.elastic.clients.elasticsearch._types.query_dsl.QueryBuilders.geoDistance() //
				.field(fieldName) //
				.distance(dist) //
	...
	private static ObjectBuilder<GeoBoundingBoxQuery> boundingBoxQuery(String fieldName, Object... values) {
		...
		GeoBoundingBoxQuery.Builder queryBuilder = co.elastic.clients.elasticsearch._types.query_dsl.QueryBuilders.geoBoundingBox() //
				.field(fieldName);
	...
	/**
	 * extract the distance string from a {@link Distance} object.
	 *
